...
public class EnemyCommanderAgent : AgentBase
{
    ...
    public void Tick(double deltaTime, SimulationSnapshot snapshot)
    {
        ...
        // Adapt tick rate: faster when losing or when few aircraft remain
        int hostileCount = snapshot.HostileAircraft.Count;
        
        // More aggressive tick rate to keep pressure on player
        // When few aircraft: tick faster to spawn reinforcements
        // When many aircraft: tick slower to let tactics play out
        _tickInterval = hostileCount switch
        {
            0 => 8.0,      // No aircraft - spawn immediately
            1 => 10.0,     // Very few - spawn quickly
            2 or 3 => 12.0, // Few - spawn regularly
            4 or 5 => 15.0, // Moderate - normal pace
            _ => 20.0      // Many - slower pace
        };
...
public class AlliedHQAgent : AgentBase
{
    ...
    public async Task RespondToPlayerMessage(string playerMessage, SimulationSnapshot snapshot)
    {
        try
        {
            var response = await RunAsync(
                ...
                resetHistory: false,  // Keep conversation history for context
                planningMaxTokens: 512);
    ...
    /// <summary>Sends an event-driven message based on game events</summary>
    public Task SendEventDrivenMessage(string messageContent, SimulationSnapshot snapshot)
    {
        try
        {
            GameLogger.Info("AI", $"AlliedHQ sending event-driven message: {messageContent}");
            
            // Send the message directly - it's already been generated by GameEventDetector
            _tools.Simulation.Comms.Queue(CommManager.CreateAlliedHQMessage(
                messageContent,
        ...
        {
            GameLogger.Error("AI", $"AlliedHQ.SendEventDrivenMessage failed: {ex.Message}", ex);
        }
...
public class IntelligenceAgent : AgentBase
{
    ...
    public Task SendEventDrivenMessage(string messageContent, SimulationSnapshot snapshot)
    {
        try
        {
            GameLogger.Info("AI", $"Intel sending event-driven message: {messageContent}");
            
            // Send the message directly - it's already been generated by GameEventDetector
            _tools.Simulation.Comms.Queue(CommManager.CreateIntelMessage(messageContent));
        }
        catch (Exception ex)
        {
            GameLogger.Error("AI", $"Intel.SendEventDrivenMessage failed: {ex.Message}", ex);
        }
...
public class AgentOrchestrator
{
    ...
    public AgentOrchestrator(
        ...
        Func<ScenarioDefinition?>? scenarioAccessor = null,
        AIMessageQueue? messageQueue = null,
        AIMessageCooldownManager? cooldownManager = null)
    {
        _client = client;
    ...
    public void Tick(double deltaTime, SimulationSnapshot snapshot)
    {
        ...
        // Process event-driven AI messages from the queue
        ProcessQueuedMessages(snapshot);
    ...
    /// <summary>
    /// Processes queued event-driven AI messages with cooldown management.
    /// </summary>
    private void ProcessQueuedMessages(SimulationSnapshot snapshot)
    {
        if (_messageQueue == null || _cooldownManager == null)
            return;

        // Discard stale routine messages
        _messageQueue.DiscardStaleMessages();

        // Try to dequeue and send a message if cooldowns allow
        var queuedMessage = _messageQueue.DequeueNextMessage();
        if (queuedMessage == null)
            return;

        // Check if we can send a message from this agent
        if (!_cooldownManager.CanSendMessage(queuedMessage.AgentName))
        {
            // Re-queue the message for later (put it back at the front)
            _messageQueue.EnqueueMessage(queuedMessage.Event, queuedMessage.AgentName, queuedMessage.MessageContent);
            return;
        }

        // Send the message through the appropriate agent
        _ = SendEventDrivenMessageAsync(queuedMessage, snapshot);
    }

    /// <summary>
    /// Sends an event-driven message through the appropriate AI agent.
    /// </summary>
    private async Task SendEventDrivenMessageAsync(QueuedMessage queuedMessage, SimulationSnapshot snapshot)
    {
        ...
        try
        {
            GameLogger.Info("AI-EVENT", $"Processing event-driven message from {queuedMessage.AgentName}: {queuedMessage.Event.EventType}");

            // Route to appropriate agent based on agent name
            switch (queuedMessage.AgentName.ToLowerInvariant())
            {
                case "alliedhq":
                    if (!AlliedHQ.IsRunning)
                    {
                        await AlliedHQ.SendEventDrivenMessage(queuedMessage.MessageContent, snapshot);
                        _cooldownManager.RecordMessage(queuedMessage.AgentName);
                    }
                    break;
                ...
                case "intel":
                    if (!Intel.IsRunning)
                    {
                        await Intel.SendEventDrivenMessage(queuedMessage.MessageContent, snapshot);
                        _cooldownManager.RecordMessage(queuedMessage.AgentName);
                    }
                    break;

                default:
                    GameLogger.Warning("AI-EVENT", $"Unknown agent name: {queuedMessage.AgentName}");
                    break;
            }
        }
        catch (Exception ex)
        {
            GameLogger.Error("AI-EVENT", $"Failed to send event-driven message from {queuedMessage.AgentName}", ex);
        }
    }
